...
// Add type definition for DotMaterial
declare global {
    namespace JSX {
...
function Scene() {
    ...
    const getThemeColors = () => {
        switch (theme) {
            case 'dark':
                return {
                    dotColor: '#FFFFFF',
                    bgColor: '#05060F', // Adjusted to match original site bg
                    dotOpacity: 0.1 // Increased opacity for visibility
                }
            case 'light':
                return {
                    dotColor: '#05060F',
    ...
    // Fixed hook usage order
    const dotMaterial = useMemo(() => {
        // @ts-ignore - Constructor type not inferred correctly by drei shaderMaterial
